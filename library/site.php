...
function makePage($options=array()) {
	...
	page('sidebar'); // preset the sidebar if one exists
 ...
 */
function page($part='', $value=null, $page='') {
	switch ($part) {
		case 'title':
			if (is_string($value)) {
		...
		case 'sidebar':
			if ($value == null) {
				$sb = get('sidebar', false);
				
				if ($sb !== false) return $sb; // already established
				...
				foreach ($searchpath as $page) {
					$sblist[] = get('parts').'/sidebar/'.get('pages').'/'.$page.'.html';
					append($path, $page, '/');
					$sblist[] = get('parts').'/sidebar/'.get('pages').'/'.$path.'.html';
				}
				
				foreach ($sblist as $sb) {
					if (file_exists($sb) && is_file($sb)) {
						set('sidebar', $sb);
						return $sb;
					}
				}
				$sb = get('sidebar', get('parts').'/sidebar/'.get('default-sidebar', false));
